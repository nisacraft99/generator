...
- Prefer multiple focused test cases over one large merged test case.
...
- A manager negative test for MM may log in as Manager, but must not invent a Manager Console -> MM path if such path does not exist.
...
def evaluate_navigation(us_id_value: str, cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for tc in cases:
        ...
        if has_nav:
            valid_cases += 1
            # Compare against expected prefix length of actual or expected min
            # Best for your use case: actual must match beginning of expected OR expected beginning of actual
            min_len = min(len(actual), len(expected))
            is_correct = (actual[:min_len] == expected[:min_len])

            # stricter target check if expected length <= actual length
