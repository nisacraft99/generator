...
def infer_nodes_from_step_text(step_text: str, expected_text: str) -> List[str]:
    """
    Conservative fallback inference of UI node IDs from natural-language steps.

    Important design choice:
    - Explicit ui_node_id values generated by the LLM are preferred elsewhere.
    - This inference is only a fallback when a step has no usable ui_node_id.
    - It is intentionally scoped by module (SM/TM) and function level
      (Meeting vs Action), so a word like "delete" cannot add every delete node
      from SM, TM, SM actions and TM actions at once.
    """
    text = normalize_text(f"{step_text} {expected_text}")
    raw_text = f"{step_text} {expected_text}"
    found: List[str] = []
    valid_ids = _node_ids()
    ...
    module_scope = _detect_module_scope(raw_text)
    function_scope = _detect_function_scope(raw_text)

    # Workspaces: require explicit workspace/console wording to avoid matching
    # ordinary business words like performance or operations.
    ...
    # Old names only for backwards compatibility with older generated outputs.
    ...
    # Module-scoped meeting nodes.
    if module_scope == "SM":
        add("OPT-MM")
        if "dashboard" in text:
            add("SCR-MM-DASHBOARD")
        if re.search(r"\bdetail\b|\bdetails\b|\bid link\b|\bexisting sm\b|\bselected sm\b|\bopen.*sm\b", text):
            add("SCR-MM-DETAIL")

        if function_scope == "MEETING":
            if re.search(r"\bcreate\b", text):
                if "button" in text or "click" in text:
                    add("EL-MM-CREATE")
                if "popup" in text or "pop up" in text or "modal" in text or "appears" in text:
                    add("MOD-MM-CREATE")
            if re.search(r"\bedit\b", text):
                if "button" in text or "click" in text:
                    add("EL-MM-EDIT")
                if "popup" in text or "pop up" in text or "modal" in text or "opens" in text:
                    add("MOD-MM-EDIT")
            if re.search(r"\bdelete\b", text):
                if "button" in text or "click" in text:
                    add("EL-MM-DELETE")
                if "popup" in text or "pop up" in text or "confirmation" in text or "sure" in text or "opens" in text:
                    add("MOD-MM-DELETE")
            if "search" in text:
                add("EL-MM-SEARCH")
                if "button" in text or "click" in text:
                    add("EL-MM-SEARCH-BTN")
            if "reset" in text:
                add("EL-MM-RESET")
            if "id" in text and ("link" in text or "list" in text or "dashboard" in text):
                add("EL-MM-ID")
            if "list" in text or "table" in text:
                add("COMP-MM-LIST")

        if function_scope == "ACTION":
            if "list" in text:
                add("COMP-MM-ACTION-LIST")
            if "id" in text or "detail" in text:
                add("EL-MM-ACTION-ID")
                add("SCR-MM-ACTION-DETAIL")
            if re.search(r"\badd action\b|\bcreate action\b", text):
                if "button" in text or "click" in text:
                    add("EL-MM-ADD-ACTION")
                if "popup" in text or "pop up" in text or "modal" in text or "window" in text or "appears" in text:
                    add("MOD-MM-ACTION-CREATE")
            if "edit" in text:
                if "button" in text or "click" in text:
                    add("EL-MM-ACTION-EDIT")
                if "popup" in text or "pop up" in text or "modal" in text or "window" in text or "opens" in text:
                    add("MOD-MM-ACTION-EDIT")
            if "delete" in text:
                if "button" in text or "click" in text:
                    add("EL-MM-ACTION-DELETE")
                if "popup" in text or "pop up" in text or "confirmation" in text or "sure" in text or "opens" in text:
                    add("MOD-MM-ACTION-DELETE")

    elif module_scope == "TM":
        add("OPT-AM")
        if "dashboard" in text:
            add("SCR-AM-DASHBOARD")
        if re.search(r"\bdetail\b|\bdetails\b|\bid link\b|\bexisting tm\b|\bselected tm\b|\bopen.*tm\b", text):
            add("SCR-AM-DETAIL")
        ...
        if function_scope == "ACTION":
            ...
            if "edit" in text:
                if "button" in text or "click" in text:
                    add("EL-AM-ACTION-EDIT")
                if "popup" in text or "pop up" in text or "modal" in text or "window" in text or "opens" in text:
                    add("MOD-AM-ACTION-EDIT")
            if "delete" in text:
                if "button" in text or "click" in text:
                    add("EL-AM-ACTION-DELETE")
                if "popup" in text or "pop up" in text or "confirmation" in text or "sure" in text or "opens" in text:
                    add("MOD-AM-ACTION-DELETE")

    else:
        # Non-meeting areas. Keep this conservative.
        if "calendar" in text:
            add("OPT-CALENDAR")
            add("SCR-CALENDAR")
            if "left" in text:
                add("EL-CALENDAR-LEFT")
            if "right" in text:
                add("EL-CALENDAR-RIGHT")
            if "search" in text:
                add("EL-CALENDAR-SEARCH")
                if "button" in text or "click" in text:
        ...
        if "evaluate employees" in text or "employee evaluation" in text:
            add("OPT-EVALUATE")
            if "dashboard" in text:
            ...
            if "search" in text:
                add("EL-EVAL-SEARCH")
                if "button" in text or "click" in text:
                    add("EL-EVAL-SEARCH-BTN")
            if "location" in text:
                add("EL-EVAL-LOCATION")
        ...
        if "my evaluations" in text or "my evaluation" in text:
            ...
            if "list" in text:
                add("COMP-MY-EVAL-LIST")
            if "detail" in text or "open an evaluation" in text:
                add("EL-MY-EVAL-ID")
                add("SCR-MY-EVAL-DETAIL")
            if "back" in text:
                add("EL-MY-EVAL-BACK")
            if "appeal" in text:
                add("EL-MY-EVAL-APPEAL")
                if "popup" in text or "pop up" in text or "modal" in text or "appears" in text:
                    add("MOD-EVAL-APPEAL")
            if "comment" in text:
                add("FIELD-EVAL-APPEAL-COMMENT")
            if "save" in text:
                add("EL-EVAL-APPEAL-SAVE")
            if "cancel" in text:
                add("EL-EVAL-APPEAL-CANCEL")
